...
extern "C" {
    ...
    /// macOS. Defined in `src/mic_permission.m`. Registers our bundle
    /// with TCC so the WebView's later `getUserMedia` finds a cached
    /// grant when it consults macOS for permission.
