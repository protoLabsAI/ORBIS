...
async def run_bot(user_id: str = "default", *, transport: LocalAudioTransport | None = None) -> None:
    ...
    # Chance the opening ack is freshly generated by the micro LLM (a
    # "surprise" custom ack) instead of the canned pool — only on tools that
    # take a beat (medium/slow), so the ~0.5s generation still lands before
    # the result. Fast tools always use the instant pool. Mirrors
    # MICRO_ACK_LLM_CHANCE for the between-turn ack.
    _opening_llm_chance = float(os.environ.get("OPENING_ACK_LLM_CHANCE", "0.25"))
