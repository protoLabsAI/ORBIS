...
| `OPENING_ACK_LLM_CHANCE` | `0.25` | Chance the opening acknowledgement spoken the instant a tool call starts ("on it", "let me check") is freshly generated by the micro model for a surprise custom line instead of the canned pool. Only on tools that take a beat (so the generation lands before the result); fast tools always use the instant pool. `0` = always canned. |
