...
import type { ParamMap, VoiceState } from './types';
...
/**
 * Coerce an arbitrary composed param map into a complete OrbBasePreset.
 * `.orbis` definitions aren't forced to declare every core key — missing
 * ones fall back to sane neutral values (brand colors, unity-ish motion)
 * so stateSnapshot() can never produce NaNs from a sparse palette.
 */
export function coerceBasePreset(params: ParamMap): OrbBasePreset {
  const num = (v: unknown, d: number): number =>
    typeof v === 'number' && Number.isFinite(v) ? v : d;
  const col = (v: unknown, d: string): string =>
    typeof v === 'string' && v.length > 0 ? v : d;
  return {
    density: num(params.density, 1.0),
    atmosphereGlow: num(params.atmosphereGlow, 0.5),
    speed: num(params.speed, 1.0),
    chromaticAberration: num(params.chromaticAberration, 0.0),
    asymmetry: num(params.asymmetry, 0.0),
    orbRotation: num(params.orbRotation, 0.5),
    primaryEnergy: col(params.primaryEnergy, '#9b87f2'),
    secondaryEnergy: col(params.secondaryEnergy, '#6366f1'),
  };
}
